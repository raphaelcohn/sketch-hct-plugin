...
export function compile_typescript(root_folder_path, working_directory_folder_path)
{
	...
	if (!stat.isFile())
	{
		throw new Error(`Typescript configuration file ${typescript_configuration_file_path} is not a file`)
	}

	if (!stat.is_readable())
	{
		throw new Error(`Typescript configuration file ${typescript_configuration_file_path} is not readable`)
	}
